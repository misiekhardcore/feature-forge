...
describe("DefaultAgentGovernancePolicy", () => {
  ...
  describe("resolvePermissions", () => {
    it("returns null (unrestricted) when toolRestrictions is empty", async () => {
      const spec = makeSpec({ toolRestrictions: {} });
      const perms = await policy.resolvePermissions(spec);
      expect(perms.allowedTools).toBeNull();
    });

    it("returns allowedTools when toolRestrictions is non-empty", async () => {
      const spec = makeSpec({ toolRestrictions: { read: [], grep: [] } });
      const perms = await policy.resolvePermissions(spec);
