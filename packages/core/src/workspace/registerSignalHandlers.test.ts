...
describe("registerSignalHandlers", () => {
  ...
  it("only destroys session-scoped paths, not all registry workspaces", async () => {
    ...
    // Track only A and B as session paths
    workspaceManager.trackPath("/tmp/ws-A");
    workspaceManager.trackPath("/tmp/ws-B");

    const destroySpy = vi.spyOn(provider, "destroyWorkspace");

    cleanupWorkspaces(workspaceManager, "SIGINT", workspaceManager.listSessionPaths(), exit);
    await vi.runAllTimersAsync();
