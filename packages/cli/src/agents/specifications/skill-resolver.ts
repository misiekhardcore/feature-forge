...
/**
 * Known directories to scan for skill definitions, in priority order.
 *
 * - `~/.agents/skills/` — user-level custom skills (used by agent skills ecosystem)
 * - `~/.pi/agent/skills/` — pi's built-in skill directory
 * - `.forge/skills/` — project-specific forge skills
 */
 ...
 */
function scanSkillDirectory(dirPath: string, nameMap: Map<string, string>): void {
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      const name = parseSkillName(skillDir);
...
export function resolveSkillPaths(
  ...
  const names = resolveEffectiveNames(allSkills, skills, excludedSkills);
 ...
 */
export function discoverAllSkills(): Map<string, string> {
  const nameMap = new Map<string, string>();

  for (const dir of getSkillDirectories()) {
    scanSkillDirectory(dir, nameMap);
  }
 ...
 */
export function resolveEffectiveNames(
