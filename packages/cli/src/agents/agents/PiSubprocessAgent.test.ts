...
// Store onEvent callbacks so tests can fire events through the internal listener.
// Shared reference: vi.mock factory pushes callbacks here, tests call fireEvent.
...
const { MockRpcClient, getRpcMock, resetRpcMock } = vi.hoisted(() => {
  ...
  function reset() {
    // Clear stored callbacks on reset.
...
describe("PiSubprocessAgent", () => {
  ...
  describe("executeTask", () => {
    ...
    it("executes task successfully and extracts assistant text", async () => {
      ...
      // Fire a message_end with assistant text, then agent_end to resolve.
    ...
    it("handles empty content gracefully (no events before agent_end)", async () => {
      ...
      // Fire agent_end without any message_end — result should be empty string.
      fireEvent({ type: "agent_end" });
    ...
    it("filters events by type and role and content presence", async () => {
      ...
      // message_start — should be ignored (only message_end extracted)
      fireEvent({
        type: "message_start",
        message: { role: "assistant", content: [{ type: "text", text: "should skip" }] },
      });
      // message_end with user role — should be ignored
      fireEvent({
        type: "message_end",
        message: { role: "user", content: [{ type: "text", text: "should skip" }] },
      });
      // message_end without content — should be ignored
    ...
    describe("streaming (prompt + onEvent)", () => {
      it("subscribes to onEvent when options.onEvent is provided", async () => {
        ...
        // Send a stream event through both listeners by firing agent_end.
        // The internal listener (callbacks[0]) resolves the promise;
        // the external listener (callbacks[1]) is the test callback.
        fireEvent({ type: "agent_end" });
        await resultPromise;

        // The external callback was subscribed (2 onEvent calls: internal + external).
      ...
      it("does not subscribe external onEvent when no callback provided", async () => {
        ...
        // Only the internal listener was registered (1 call to onEvent).
      ...
      it("subscribes external callback before calling prompt", async () => {
        ...
        // Both onEvent calls (internal + external) happened before prompt.
      ...
      it("calls prompt with the message and images", async () => {
        ...
        const resultPromise = agent.executeTask("hello", {
          images: [imageContent],
        });
      ...
      it("unsubscribes after successful execution", async () => {
        ...
        // The unsubscribe functions (returned by onEvent) were called.
        // onEvent was called 2 times (internal + external).
        expect(getRpcMock().onEvent).toHaveBeenCalledTimes(2);
      });
      ...
      it("unsubscribes after failed execution", async () => {
        ...
        // Even on failure, both unsubscribe functions are called.
        expect(getRpcMock().onEvent).toHaveBeenCalledTimes(2);
      });
      ...
      it("unsubscribes when prompt rejects with onEvent listeners already active", async () => {
        ...
        expect(agent.status).toBe(AgentStatus.Failed);
        // Both listeners were subscribed then unsubscribed.
        expect(getRpcMock().onEvent).toHaveBeenCalledTimes(2);
      });
