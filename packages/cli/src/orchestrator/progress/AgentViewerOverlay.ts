...
+   *
  ...
  loadConversationEvents(agentId: string, fromIndex: number, toIndex: number): AgentEvent[] {
    ...
    try {
      ...
      // If the file is empty (single empty line after trim returns [""]), return [].
  ...
  private renderList(width: number): string[] {
    ...
    for (let index = 0; index < entries.length; index++) {
      ...
      const { char: icon, color: iconColor } = StreamHelpers.getStatusIcon(
        entry.status,
        entry.passed,
      );
  ...
  private renderDetail(width: number): string[] {
    ...
    const { char: icon } = StreamHelpers.getStatusIcon(entry.status, entry.passed);
  ...
  private renderConversationTurns(events: AgentEvent[], width: number): string[] {
    ...
    const flushMessage = (): void => {
      if (pendingMessage && pendingMessage.content.length > 0) {
        const roleColor: "userMessageText" | "accent" =
          pendingMessage.role === "user" ? "userMessageText" : "accent";
    ...
    for (const event of events) {
      if (event.type === "message_start") {
        ...
        if (pendingMessage) {
          ...
          pendingMessage.content = StreamHelpers.extractMessageText(typed["message"]);
        }
        ...
        const args =
          "args" in typed && typed["args"] !== undefined
            ? StreamHelpers.serializeToolArgs(typed["args"])
            : undefined;
  ...
  private renderToolCall(
    ...
    const { char: resultIcon, color: resultIconColor } =
      StreamHelpers.getStatusIcon(toolDisplayStatus);
    ...
    if (turn.toolArgs) {
      const innerArgsWidth = Math.max(10, width - 8);
      const truncatedArgs =
        turn.toolArgs.length > innerArgsWidth
          ? turn.toolArgs.slice(0, innerArgsWidth - 3) + "..."
          : turn.toolArgs;
  ...
  private static formatDetail(event: Record<string, unknown>, eventType: string): string {
    switch (eventType) {
      ...
      case "message_start": {
        const role = StreamHelpers.getNestedString(event, "message", "role");
        return role.slice(0, 80);
      }

      case "message_update":
      case "message_end": {
        const text = StreamHelpers.extractMessageText(event.message);
      ...
      case "tool_execution_start": {
        ...
        if ("args" in event && event.args !== undefined) {
          const serialized = StreamHelpers.serializeToolArgs(event.args);
