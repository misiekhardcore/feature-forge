...
export class GitWorktreeProvider extends WorkspaceProvider {
  ...
  private async resolveSymlinks(
    ...
    // Append the symlink paths to the repository-local git exclude file so git
    // never stages the worktree symlinks, regardless of .gitignore pattern
    // matching (trailing-slash patterns only match directories, not symlinks).
    // info/exclude is never committed and applies independently of .gitignore.
    const excludePaths = unique.filter(
      (s) => !(s === ".forge/worktrees" || s.startsWith(".forge/worktrees/")),
    );

    if (excludePaths.length > 0) {
      const excludeFile = await this.resolveExcludeFile(worktreePath);
      const entries = excludePaths.map((s) => `/${s.replace(/\/$/, "")}`).join("\n");
      try {
        appendFileSync(
          excludeFile,
        ...
        logger.warn("Could not write git exclude file", { excludeFile, error });
      }
    }
  }

  /**
   * Resolve the repository-local git exclude file (`info/exclude`) that applies
   * to the given worktree. In linked worktrees `.git` is a file pointing at the
   * per-worktree git dir, so `info/exclude` cannot be addressed as
   * `<worktree>/.git/info/exclude` - it lives in the common git dir and is
   * resolved by git itself.
   */
  private async resolveExcludeFile(worktreePath: string): Promise<string> {
    try {
      const output = await this.execCommand("git", ["rev-parse", "--git-path", "info/exclude"]);
      const resolved = output.trim();
      if (resolved.length > 0) {
        // git may return the path relative to its cwd (the main checkout)
        return resolve(this.repoRoot, resolved);
      }
    } catch (error) {
      logger.debug("Could not resolve git exclude path", { worktreePath, error });
    }
    return resolve(worktreePath, ".git", "info", "exclude");
  }
