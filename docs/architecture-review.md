...
Verified: `npx tsx -e "import('@feature-forge/shared')..."` → `import FAIL: The requested module './config' does not provide an export named 'AgentConfig'`. `validate-flow-json.ts` survives only because it imports `../src/helpers` relatively from `packages/core/scripts/` (in-package path, since 0175f9b4 - superseded: the E1/E2 exports map additionally forbids `/src/` deep imports through `@feature-forge/core` subpaths, which relative in-package imports never use); both flow scripts now import relatively, so the style divergence this note described no longer exists.
