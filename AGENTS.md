...
- **Style (OOP)**: ❌ Avoid functional-style modules - exported free functions or module-level arrow constants as the unit of design $\to$ ✅ This project is object-oriented: expose modules as classes (instance classes for stateful collaborators; static-only utility classes with a private constructor for stateless helpers, per ADR 0017 - precedents: `ExpressionEvaluator`, `FlowValidation`, `SkillResolver`, `ResultPathWalker`, `TemplateResolver`). Name class files after their primary class (`ResultPathWalker` lives in `ResultPathWalker.ts`, `TemplateResolver` in `TemplateResolver.ts`). Free functions are acceptable only as private module-local helpers or test fixtures, never as the public module surface.
